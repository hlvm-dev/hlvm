...
/**
 * Executes a command with automatic process cleanup
 */
export async function execCommand(cmd, args, options = {}) {
  ...
  if (options.stdin === "piped") {
    return execWithStdin(command, options.input);
  }
...
async function execWithStdin(command, input) {
...
/**
 * Creates a cleanup registry for multiple resources
 */
export class CleanupRegistry {
  ...
  async cleanup() {
    ...
    if (errors.length > 0) {
      console.error(`Cleanup had ${errors.length} error(s):`, errors);
    }
  }
}